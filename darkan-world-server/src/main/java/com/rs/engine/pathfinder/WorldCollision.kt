...
object WorldCollision {
    ...
    private val chunkInited = BooleanArray(CHUNK_SIZE * CHUNK_SIZE * 4)
    ...
    fun clearChunk(chunkCollisionHash: Int) {
        synchronized(LOCK) {
            ...
            chunkInited[chunkCollisionHash] = false
        }
    }

    @JvmStatic
    fun initChunk(chunkId: Int) {
        synchronized(LOCK) {
            if (allFlags[chunkId] == null) allFlags[chunkId] = IntArray(64)
            chunkInited[chunkId] = true
        }
    }
    ...
    fun getFlags(tile: Tile): Int {
        synchronized(LOCK) {
            ...
            if (!chunkInited[chunkId]) return -1
            return allFlags[chunkId]?.get(tile.xInChunk or (tile.yInChunk shl 3)) ?: -1
        }
    }
    ...
    fun getFlags(x: Int, y: Int, plane: Int): Int {
        synchronized(LOCK) {
            ...
            if (!chunkInited[chunkId]) return -1
            return allFlags[chunkId]?.get(x and 7 or ((y and 7) shl 3)) ?: -1
        }
    }
    ...
    fun removeFlag(tile: Tile, flag: Int) {
        synchronized(LOCK) {
            val chunkId = tile.chunkId
            val chunk = allFlags[chunkId] ?: return
            chunk[tile.xInChunk or (tile.yInChunk shl 3)] = chunk[tile.xInChunk or (tile.yInChunk shl 3)] and flag.inv()
        }
    }
