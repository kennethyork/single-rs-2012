...
public class HighscoresManager extends DBItemManager {
	...
	public void saveSync(Player player) {
		...
		try {
			if (Settings.getConfig().isSinglePlayer()) {
				LocalFileStore.writeAtomic("highscores/" + player.getUsername() + ".json", JsonFileManager.toJson(new Highscore(player)));
				return;
			}
	...
	public void getPlayerAtPosition(int rank, Consumer<Highscore> top) {
		if (Settings.getConfig().isSinglePlayer()) {
			try {
				File directory = LocalFileStore.file("highscores");
