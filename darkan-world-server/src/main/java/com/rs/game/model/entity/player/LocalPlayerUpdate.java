...
public final class LocalPlayerUpdate {
	...
	private void applyBodyGlowMask(Player p, OutputStream data) {
		...
		data.writeByteInverse(p.getNextBodyGlow().getGreenAdd());
		data.writeByteSubtract(p.getNextBodyGlow().getBlueAdd());
		data.writeByteSubtract(p.getNextBodyGlow().getScalar());
		data.writeShortAdd(0);
	...
	private void applyHitsMask(Player p, OutputStream data) {
		data.writeByteSubtract(p.getNextHits().size());
		for (Hit hit : p.getNextHits()) {
			boolean interactingWith = hit.interactingWith(player, p);
			if (hit.getText() == null && hit.missed() && !interactingWith) {
				data.writeSmart(32766);
				data.writeByteInverse(0);
			} else if (hit.getSoaking() != null) {
                if (hit.getText() != null || hit.getDamage() > Short.MAX_VALUE/2 || hit.getSoaking().getDamage() > Short.MAX_VALUE/2) {
                    data.writeSmart(32764);
                    data.writeSmart(hit.getMark(player, p));
                    data.writeString(hit.getText() != null ? hit.getText() : Utils.formatNumber(hit.getDamage()));
                    data.writeSmart(hit.getSoaking().getMark(player, p));
                    data.writeString(Utils.formatNumber(hit.getSoaking().getDamage()));
                } else {
                ...
                if (hit.getText() != null || hit.getDamage() > Short.MAX_VALUE/2) {
                    data.writeSmart(32765);
                    data.writeSmart(hit.getMark(player, p));
                    data.writeString(hit.getText() != null ? hit.getText() : Utils.formatNumber(hit.getDamage()));
                } else {
                    data.writeSmart(hit.getMark(player, p));
		...
		for (HitBar bar : p.getNextHitBars()) {
			...
			if (display) {
				...
				data.writeByteSubtract(perc);
				if (toPerc != perc)
					data.writeByteAdd(toPerc);
			}
		}
	}

	private void applyFaceEntityMask(Player p, OutputStream data) {
		data.writeShortAdd(p.getNextFaceEntity() == -2 ? p.getLastFaceEntity() : p.getNextFaceEntity());
	}

	private void applyFaceDirectionMask(Player p, OutputStream data) {
		data.writeShortAdd(p.getFaceAngle()); // also works as face tile as dir
		// calced on setnextfacetile
	}

	private void applyMoveTypeMask(Player p, OutputStream data) {
		data.writeByteSubtract(p.getMovementType() == Entity.MoveType.RUN ? 2 : 1);
	}

	private void applyTemporaryMoveTypeMask(Player p, OutputStream data) {
		data.writeByteInverse(p.getTemporaryMoveType().getId());
	}

	private void applyGraphicsMask1(Player p, OutputStream data) {
		data.writeShortLittle(p.getNextSpotAnim1().getId());
		data.writeInt(p.getNextSpotAnim1().getSettingsHash());
		data.writeByteAdd(p.getNextSpotAnim1().getSettings2Hash());
	}

	private void applyGraphicsMask2(Player p, OutputStream data) {
		data.writeShort(p.getNextSpotAnim2().getId());
		data.writeIntInverseMiddle(p.getNextSpotAnim2().getSettingsHash());
		data.writeByteSubtract(p.getNextSpotAnim2().getSettings2Hash());
	}

	private void applyGraphicsMask3(Player p, OutputStream data) {
		data.writeShortAddLittle(p.getNextSpotAnim3().getId());
		data.writeIntInverseMiddle(p.getNextSpotAnim3().getSettingsHash());
		data.writeByteAdd(p.getNextSpotAnim3().getSettings2Hash());
	}

	private void applyGraphicsMask4(Player p, OutputStream data) {
		data.writeShortLittle(p.getNextSpotAnim4().getId());
		data.writeIntLittle(p.getNextSpotAnim4().getSettingsHash());
		data.writeByteSubtract(p.getNextSpotAnim4().getSettings2Hash());
	}
	...
	private void applyAppearanceMask(Player p, OutputStream data) {
		...
		data.writeByteInverse(renderData.length);
		data.writeBytes(renderData);
	}

	private void applyForceMovementMask(Player p, OutputStream data) {
		data.writeByteInverse(p.getNextForceMovement().getDiffX1());
		data.writeByteSubtract(p.getNextForceMovement().getDiffY1());
		data.writeByteAdd(p.getNextForceMovement().getDiffX2());
		data.writeByteInverse(p.getNextForceMovement().getDiffY2());
		data.writeShortAddLittle(p.getNextForceMovement().getStartClientCycles());
		data.writeShortLittle(p.getNextForceMovement().getSpeedClientCycles());
		data.writeShortAdd(p.getNextForceMovement().getDirection());
	}
