...
public final class LocalNPCUpdate {
	...
	private void applyForceMovementMask(NPC n, OutputStream data) {
		data.writeByteSubtract(n.getNextForceMovement().getDiffX1());
		data.writeByteAdd(n.getNextForceMovement().getDiffY1());
		data.writeByteInverse(n.getNextForceMovement().getDiffX2());
		data.writeByteAdd(n.getNextForceMovement().getDiffY2());
		data.writeShortLittle(n.getNextForceMovement().getStartClientCycles());
		data.writeShortLittle(n.getNextForceMovement().getSpeedClientCycles());
		data.writeShortAddLittle(n.getNextForceMovement().getDirection());
	}
	...
	private void applyFaceTileMask(NPC n, OutputStream data) {
		...
		data.writeShortAddLittle((tile.getX() << 1) + 1);
		data.writeShortLittle((tile.getY() << 1) + 1);
	}

	private void applyHitMask(NPC n, OutputStream data) {
		data.writeByteAdd(n.getNextHits().size());
		...
		data.writeByteAdd(n.getNextHitBars().size());
	...
	private void applyFaceEntityMask(NPC n, OutputStream data) {
		data.writeShortLittle(n.getNextFaceEntity() == -2 ? n.getLastFaceEntity() : n.getNextFaceEntity());
	}
	...
	private void applyGraphicsMask4(NPC n, OutputStream data) {
		data.writeShortAdd(n.getNextSpotAnim4().getId());
		data.writeIntInverseMiddle(n.getNextSpotAnim4().getSettingsHash());
		data.writeByteAdd(n.getNextSpotAnim4().getSettings2Hash());
	}

	private void applyGraphicsMask3(NPC n, OutputStream data) {
		data.writeShortLittle(n.getNextSpotAnim3().getId());
		data.writeIntLittle(n.getNextSpotAnim3().getSettingsHash());
		data.writeByte(n.getNextSpotAnim3().getSettings2Hash());
	}

	private void applyGraphicsMask2(NPC n, OutputStream data) {
		data.writeShortLittle(n.getNextSpotAnim2().getId());
		data.writeInt(n.getNextSpotAnim2().getSettingsHash());
		data.writeByteAdd(n.getNextSpotAnim2().getSettings2Hash());
	}

	private void applyGraphicsMask1(NPC n, OutputStream data) {
		data.writeShort(n.getNextSpotAnim1().getId());
		data.writeIntInverseMiddle(n.getNextSpotAnim1().getSettingsHash());
		data.writeByteInverse(n.getNextSpotAnim1().getSettings2Hash());
	}
