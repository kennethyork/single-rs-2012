...
import static com.rs.game.World.sendSpotAnim;
import static com.rs.engine.variables.VarBitPlayer.bank_show_equipscreen;
import static com.rs.engine.variables.VarBitPlayer.barrows_killed_count;
...
public class NPC extends Entity {
	...
	public void processNPC() {
		...
		Sap.tickSapDecay(this);
		Leech.tickLeechDecay(this);
		if (isForceWalking())
	...
	public void spawn() {
		if (deleted)
			return;
		if (!ChunkManager.getChunk(getRespawnTile().getChunkId()).isLoadedMapData()) {
			deleted = true;
			return;
		}
	...
	public void drop(Player killer, boolean verifyCombatDefs) {
		try {
			...
			if (BarrowsController.barrowsKCNPCs.contains(getId())) {
				...
				killer.getVars().setVarBit(barrows_killed_count, killer.getBarrowsKillCount() + killer.getKilledBarrowBrothersCount());
			}
	...
	public void sendDrop(Player player, Item item) {
		...
		GroundItem gItem = new GroundItem(item, Tile.of(getCoordFaceX(size), getCoordFaceY(size), getPlane()), dropTo.getUsername(), GroundItem.GroundItemType.INVISIBLE);

		NPCDropEvent event = new NPCDropEvent(dropTo, this, gItem);
		PluginManager.handle(event);
		if (event.getItem() == null || event.getItem().getId() == -1) return;
		World.addGroundItem(event.getItem());
	}
	...
	public static void displayDropsFor(Player player, int npcId, int npcAmount) {
		...
		AsyncTaskExecutor.execute(() -> {
			...
			player.getVars().setVarBit(bank_show_equipscreen, 0);
