	...
	public void removeHitpoints(Hit hit) {
		...
		if (hitpoints <= 0)
			...
			if (player.getEquipment().getAmuletId() == 11090 && player.getHitpoints() <= player.getMaxHitpoints() * 0.2) {
				...
				player.sendMessage("Your phoenix necklace heals you, but is destroyed in the process.");
			}
...
//				return DumbRouteFinder.addDumbPathfinderSteps(this, target, getClipType());
	...
	public void heal(int amount, int extra) {
		setHitpoints((hitpoints + amount) >= (getMaxHitpoints() + extra) ? (getMaxHitpoints() + extra) : (hitpoints + amount));
	}
