...
public class InterfaceManager {
	...
	private final BiMap<Integer, Integer> openedInterfaces = Maps.synchronizedBiMap(HashBiMap.create());
...
//				Familiar.sendLeftClickOption(player);
	...
	private void clearChilds(int parentInterfaceId) {
		openedInterfaces.keySet().removeIf(key -> key >> 16 == parentInterfaceId);
	}
	...
	public boolean topOpen(int interfaceId) {
		if (interfaceId == top)
			return true;
		return openedInterfaces.containsValue(interfaceId);
	}
