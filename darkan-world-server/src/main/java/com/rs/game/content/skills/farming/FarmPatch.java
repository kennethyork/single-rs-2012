...
public class FarmPatch {
	...
	private void useItem(Player player, GameObject object, Item item) {
		...
		if (produce != null) {
			...
			player.anim(SEED_DIBBING_ANIMATION);
			player.getInventory().deleteItem(produce.seedId, seedCount);
			seed = produce;
			updateVars(player);
			if (location == PatchLocation.Herblore_Habitat_flower || location == PatchLocation.Herblore_Habitat_bush || location == PatchLocation.Herblore_Habitat_fruit_tree) {
				JadinkoType.checkJadinkoAttraction(player);
			}
			return;
		}
		switch(item.getId()) {
			...
			case 6036:
				...
				player.anim(CURE_PLANT_ANIMATION);
				item.setId(229);
				player.getInventory().refresh();
				diseased = false;
				updateVars(player);
			...
			case 18682:
				...
				player.anim(WATERING_ANIMATION);
				watered = true;
				updateVars(player);
				break;
			case 952:
