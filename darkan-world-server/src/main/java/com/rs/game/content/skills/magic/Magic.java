...
public class Magic {
    ...
    public static void sendLunarTeleportGroupSpell(Player player, int level, double xp, Tile tile, RuneSet runes) {
        var targets = player.queryNearbyPlayersByTileRange(3, p ->
                p != null
                        && p != player
                        && !p.isDead()
        );
        for (Player target : targets)
            sendTeleportSpell(target, 9606, -1, 1685, -1, level, 0.0, tile, 5, true, TeleType.MAGIC, null);
        sendTeleportSpell(player, 9606, -1, 1685, -1, level, xp, tile, 5, true, TeleType.MAGIC, runes, null);
    }
	...
	public static boolean sendTeleportSpell(final Player player, int upEmoteId, final int downEmoteId, int upGraphicId, final int downGraphicId, int level, final double xp, final Tile tile, int delay, final boolean randomize, final TeleType teleType, RuneSet runes, Runnable onCast, Runnable onArrive) {
        Teleport tele = new Teleport(Tile.of(player.getTile()), tile, teleType, () -> {
			...
			if (onCast != null)
				onCast.run();
			if (upEmoteId != -1)
				player.anim(upEmoteId);
			if (upGraphicId != -1)
				player.spotAnim(new SpotAnim(upGraphicId));
	...
	public static void npcTeleport(NPC npc, int upEmoteId, final int downEmoteId, int upGraphicId, final int downGraphicId, final Tile tile, int delay, final boolean randomize, Consumer<NPC> onArrive) {
		...
		if (upEmoteId != -1)
			npc.anim(upEmoteId);
		if (upGraphicId != -1)
			npc.spotAnim(new SpotAnim(upGraphicId));
		WorldTasks.delay(delay, () -> {
			...
			npc.tele(teleTile);
			if (downEmoteId != -1)
				npc.anim(downEmoteId == -2 ? -1 : downEmoteId);
			if (downGraphicId != -1)
				npc.spotAnim(new SpotAnim(downGraphicId));
			if (onArrive != null)
				onArrive.accept(npc);
