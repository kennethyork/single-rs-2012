...
public class Lander {
	...
	private static final int AUTO_GAME = Settings.getConfig().isSinglePlayer() ? 1 : (Settings.getConfig().isDebug() ? 2 : 15);
	private static final int MINIMUM_PLAYERS = Settings.getConfig().isSinglePlayer() ? 1 : 5;
	...
	public class LobbyTimer extends Task {
		...
		@Override
		public void run() {
			if ((seconds == 0 && lobby.size() >= MINIMUM_PLAYERS) || lobby.size() >= AUTO_GAME)
				passPlayersToGame();
			else if (seconds == 0)
				seconds = TIME;
	...
	private void passPlayersToGame() {
		...
		if (Settings.getConfig().isSinglePlayer())
			for (Player player : playerList)
				SimulatedPlayerCompanionManager.ensureMinimum(player, 4, "Pest Control");
