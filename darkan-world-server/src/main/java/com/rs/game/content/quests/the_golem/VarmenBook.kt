...
@ServerStartupEvent
fun mapVarmenBook() {
    onItemClick(4616, options = arrayOf("Read")){ (player) ->
        if (player.getQuestStage(Quest.THE_GOLEM) == 2) {
            player.setQuestStage(Quest.THE_GOLEM, 3)
        }
        player.openBook(VarmenBook())
    }
}