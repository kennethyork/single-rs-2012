...
public enum Effect {
	...
	MIASMIC_BLOCK,
	TELEBLOCK("teleblock"),
	LIVID_FISH_COOKING_BOOST("Reduce The Fish I Burn Wish"),
	LIVID_PLANKS_BOOST("More Planks Please Wish"),
	LUNARS_SPELLBOOK_SWAP("Spellbook Swap") {
		@Override
		...
		public void expire(Entity entity) {
			if (entity instanceof Player player) {
				player.getCombatDefinitions().setSpellbook(CombatDefinitions.Spellbook.LUNAR);
				player.getCombatDefinitions().setAutoCastSpell(null);
				player.sendMessage("Your spellbook has returned to Lunar Spellbook.");
			}
		}
	},
	LUNARS_DISRUPTION_SHIELD("disruption shield") {
		public boolean sendWarnings() {
			return false;
		}
		...
		public void apply(Entity entity) {
			if (entity instanceof Player p) {
				p.sendMessage("You are protected by a Disruption Shield.");
			}
		}
	},
	LUNARS_DREAM("dream", true) {
		@Override
		public boolean sendWarnings() {
			return false;
		}
	...
	MAMBO_WARDING("Papa Mambo's Warding", false) {
		@Override
		public void expire(Entity entity) {
			if (entity instanceof Player player) {
				Object raw = player.getSavingAttributes().get("MAMBO_WARDING_TYPE");
				if (raw instanceof JadinkoType type) {
					player.getSavingAttributes().remove("MAMBO_WARDING_TYPE");
					player.sendMessage("<col=FF0000>Papa Mambo has stopped warding off "
							+ type.name().toLowerCase().replace('_',' ')
							+ " jadinkos!");
				}
			}
		}
		@Override public boolean sendWarnings() { return false; }
	},
