...
import static com.rs.engine.variables.VarBitPlayer.rand_current_progress;
import static com.rs.engine.variables.VarBitPlayer.rand_last_progress;
import static com.rs.engine.variables.VarBitPlayer.worldmap_modifier;
...
public class DungeonManager {
	...
	public void setResources(Room room, RoomReference reference, int chunkOffX, int chunkOffY) {
		if (party.getComplexity() >= 5 && Utils.random(3) == 0)
			...
			for (int i = 0; i < DungeonConstants.SET_RESOURCES_MAX_TRY; i++) {
				...
				int maxtier = 1;
				for (Player player : party.getTeam()) {
					int tier = DungeonUtils.getTier(player.getSkills().getLevel(Constants.THIEVING));
				...
				if (room.isCritPath()) {
					room.setThiefChest(maxtier);
				} else {
					room.setThiefChest(Utils.random(10));
				}
				World.spawnObject(new GameObject(DungeonConstants.THIEF_CHEST_LOCKED[party.getFloorType().ordinal()],
						ObjectType.SCENERY_INTERACT, ((rotation + 3) & 0x3), instance.getLocalX(chunkOffX, x),
						instance.getLocalY(chunkOffY, y), 0));
		...
		if (party.getComplexity() >= 4 && Utils.random(3) == 0)
			...
			for (int i = 0; i < DungeonConstants.SET_RESOURCES_MAX_TRY; i++) {
				...
				int maxtier = 1;
				for (Player player : party.getTeam()) {
					int tier = DungeonUtils.getTier(player.getSkills().getLevel(Constants.FARMING));
					maxtier = Math.max(maxtier, tier);
				}
				if (maxtier == 11) {
					maxtier -= 1;
				...
				if (room.isCritPath()) {
					...
					World.spawnObject(new GameObject(DungeonUtils.getFarmingResource(maxtier, party.getFloorType()),
							ObjectType.SCENERY_INTERACT, rotation, instance.getLocalX(chunkOffX, x),
							instance.getLocalY(chunkOffY, y), 0));
				} else {
					World.spawnObject(
							new GameObject(DungeonUtils.getFarmingResource(Utils.random(10), party.getFloorType()),
									ObjectType.SCENERY_INTERACT, rotation, instance.getLocalX(chunkOffX, x),
									instance.getLocalY(chunkOffY, y), 0));
				}
		...
		if (party.getComplexity() >= 3 && Utils.random(3) == 0)
			...
			for (int i = 0; i < DungeonConstants.SET_RESOURCES_MAX_TRY; i++) {
				...
				int maxtier = 1;
				for (Player player : party.getTeam()) {
					int tier = DungeonUtils.getTier(player.getSkills().getLevel(Constants.MINING));
					maxtier = Math.max(maxtier, tier);
				}
				if (maxtier == 11) {
					maxtier -= 1;
				...
				if (room.isCritPath()) {
					World.spawnObject(new GameObject(DungeonUtils.getMiningResource(maxtier, party.getFloorType()),
							ObjectType.SCENERY_INTERACT, rotation, instance.getLocalX(chunkOffX, x),
							instance.getLocalY(chunkOffY, y), 0));
				} else {
					World.spawnObject(
							new GameObject(DungeonUtils.getMiningResource(Utils.random(10), party.getFloorType()),
									ObjectType.SCENERY_INTERACT, rotation, instance.getLocalX(chunkOffX, x),
									instance.getLocalY(chunkOffY, y), 0));
				}
		...
		if (party.getComplexity() >= 2 && Utils.random(3) == 0)
			...
			for (int i = 0; i < DungeonConstants.SET_RESOURCES_MAX_TRY; i++) {
				...
				int maxtier = 1;
				for (Player player : party.getTeam()) {
					int tier = DungeonUtils.getTier(player.getSkills().getLevel(Constants.WOODCUTTING));
					maxtier = Math.max(maxtier, tier);
				}
				if (maxtier == 11) {
					maxtier -= 1;
				...
				if (room.isCritPath()) {
					World.spawnObject(new GameObject(DungeonUtils.getWoodcuttingResource(maxtier, party.getFloorType()),
							ObjectType.SCENERY_INTERACT, rotation, instance.getLocalX(chunkOffX, x),
							instance.getLocalY(chunkOffY, y), 0));
				} else {
					World.spawnObject(
							new GameObject(DungeonUtils.getWoodcuttingResource(Utils.random(10), party.getFloorType()),
									ObjectType.SCENERY_INTERACT, rotation, instance.getLocalX(chunkOffX, x),
									instance.getLocalY(chunkOffY, y), 0));
				}
				break;
		...
		WorldTasks.schedule(new Task() {
			...
			public void run() {
				for (Player player : party.getTeam()) {
					...
					player.sendMessage("Party Size:Difficulty <col=641d9e>" + party.getEffectiveTeamSize() + ":" + party.getDificulty());
	...
	public void spawnRandomNPCS(RoomReference reference) {
		...
		int numMonsters = Utils.random(1, 3+party.getEffectiveTeamSize());
		...
		if (Utils.random(4) == 0)
			if(getRoom(reference).isCritPath()){
				int maxtier = 1;
				for (Player player : party.getTeam()) {
					int tier = DungeonUtils.getTier(player.getSkills().getLevel(Constants.HUNTER));
					maxtier = Math.max(maxtier, tier);
				}
				if (maxtier == 11) {
					maxtier -= 1;
				...
				spawnNPC(reference, DungeonUtils.getHunterCreature(maxtier), 2 + Utils.getRandomInclusive(13),
						2 + Utils.getRandomInclusive(13), true, DungeonConstants.HUNTER_NPC);
			}else{
				spawnNPC(reference, DungeonUtils.getHunterCreature(Utils
						.random(DungeonConstants.HUNTER_CREATURES.length)), 2 + Utils.getRandomInclusive(13),
						2 + Utils.getRandomInclusive(13), true, DungeonConstants.HUNTER_NPC);
			}
	}
	...
	public DungeonNPC spawnNPC(int id, int rotation, Tile tile, RoomReference reference, int type) {
		...
		if (type == DungeonConstants.BOSS_NPC) {
			...
			visibleMap[reference.getRoomX()][reference.getRoomY()].addGuardian(n);
						...
						int maxtier = 1;
			for (Player player : party.getTeam()) {
				int tier = DungeonUtils.getTier(player.getSkills().getLevel(Constants.FISHING));
				maxtier = Math.max(maxtier, tier);
			}
			if (maxtier == 11) {
				maxtier -= 1;
			...
			if (room.isCritPath()) {
				n = new DungeonFishSpot(id, tile, this,
						DungeoneeringFishing.Fish.values()[maxtier]);
			} else {
				n = new DungeonFishSpot(id, tile, this,
						DungeoneeringFishing.Fish.values()[Utils
								.random(DungeoneeringFishing.Fish.values().length - 1)]);
			}
		}
	...
	public int getTargetLevel(int id, boolean boss, double multiplier) {
		...
		int randomize = party.getComplexity() * 2 * party.getEffectiveTeamSize();
		lvl -= randomize;
		lvl += Utils.random(randomize * 2);
		lvl *= party.getDifficultyRatio();
		lvl *= multiplier;
		lvl *= 1D - ((6D - party.getComplexity()) * 0.07D);
		if (party.getEffectiveTeamSize() > 2 && id != 12752 && id != 11872 && id != 11708 && id != 12865) //blink
			lvl *= 0.7;
	...
	public void exitDungeon(final Player player, final boolean logout) {
		...
		if (logout) {
			...
			player.getCombatDefinitions().removeDungeoneeringBook();
	...
	public void setWorldMap(Player player, boolean dungIcon) {
		player.getVars().setVarBit(worldmap_modifier, dungIcon ? 1 : 0);
	}
	...
	public void loadRewards() {
		...
		for (Player player : party.getTeam()) {
			...
			player.getVars().setVarBit(rand_current_progress, player.getDungManager().getCurrentProgress());
			player.getVars().setVarBit(rand_last_progress, player.getDungManager().getPreviousProgress());
