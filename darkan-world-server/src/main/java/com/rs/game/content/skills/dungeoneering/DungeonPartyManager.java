...
import com.rs.game.content.world.npcs.SimulatedPlayerCompanionManager;
...
public final class DungeonPartyManager {
	...
	public void setDefaults() {
		floor = 1;
		complexity = 6;
		difficulty = getEffectiveTeamSize();
		guideMode = false;
	}
	...
	public void setDificulty(int dificulty) {
		if (dificulty > getEffectiveTeamSize())
			dificulty = getEffectiveTeamSize();
	...
	public int getCombatLevel() {
		...
		level += SimulatedPlayerCompanionManager.combatLevelsForDungeoneering(team);
	...
	public int getAverageCombatLevel() {
		...
		for (Player player : team)
			level += player.getSkills().getCombatLevelWithSummoning();
		level += SimulatedPlayerCompanionManager.combatLevelsForDungeoneering(team);
		return level / getEffectiveTeamSize();
	}
	...
	public double getDifficultyRatio() {
		if (difficulty > getEffectiveTeamSize())
			return 1;
		return 0.7 + (((double) difficulty / (double) getEffectiveTeamSize()) * 0.3);
	}
	...
	public int getEffectiveTeamSize() {
		return Math.min(5, team.size() + SimulatedPlayerCompanionManager.countForDungeoneering(team));
	}
