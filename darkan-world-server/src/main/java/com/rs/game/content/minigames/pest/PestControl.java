...
public class PestControl {
	...
	/** Selects an autonomous target for NPC-backed Pest Control teammates. */
	public NPC getBotTarget(NPC bot) {
		NPC closestPest = null;
		long closestDistance = Long.MAX_VALUE;
		for (NPC npc : World.getNPCs()) {
			if (!(npc instanceof PestMonsters pest) || !pest.belongsTo(this) || npc.isDead() || npc.hasFinished())
				continue;
			long dx = (long) bot.getX() - npc.getX();
			long dy = (long) bot.getY() - npc.getY();
		...
		for (PestPortal portal : portals)
			if (portal != null && !portal.isLocked() && !portal.isDead() && !portal.hasFinished())
				return portal;
		return null;
	}
