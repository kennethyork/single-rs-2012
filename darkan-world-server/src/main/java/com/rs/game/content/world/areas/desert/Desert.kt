...
import com.rs.engine.variables.apmeken_transport
import com.rs.engine.variables.ics_ent_open
...
fun mapDesertInteractions() {
            ...
            when (forId(npc.id)) {
                CarpetLocation.SHANTAY_PASS ->
                    options("Where would you like to travel?") {
                        ...
                        if (player.isQuestComplete(Quest.THE_GOLEM))
                            opExec("Uzer") { player.tele(CarpetLocation.UZER.tile) }
                        if (player.isQuestComplete(Quest.DO_NO_EVIL))
    ...
    onLogin { (player) ->
        player.vars.setVarBit(apmeken_transport, 1)
        player.vars.setVarBit(apmeken_transport, 1)
        player.vars.setVarBit(apmeken_transport, 1)
        player.vars.setVarBit(ics_ent_open, 1)
    }
