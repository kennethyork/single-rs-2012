...
public class RunecraftingAltar {
	...
	public static void refreshHood(Player player) {
		int wickedEssCapacity = 100 + ((boolean) player.get("hasWickedCape") ? 25 : 0) + ((boolean) player.get("hasWickedBottom") ? 25 : 0) + ((boolean) player.get("hasWickedTop") ? 25 : 0);
		player.getPackets().setIFText(WICKED_HOOD_INTER, 139, "" + player.getDailySubI("wickedEss", wickedEssCapacity));
		int wickedTelesCapacity = (2 + ((boolean) player.get("hasWickedCape") && (boolean) player.get("hasWickedBottom") && (boolean) player.get("hasWickedTop") ? 1 : 0));
		player.getPackets().setIFText(WICKED_HOOD_INTER, 134, "" + player.getDailySubI("wickedTeles", wickedTelesCapacity));
		int wickedRunesCapacity = 1 + (player.getUsedElementalTalisman() ? 1 : 0) + ((boolean) player.get("hasWickedCape") ? 1 : 0) + ((boolean) player.get("hasWickedBottom") ? 1 : 0) + ((boolean) player.get("hasWickedTop") ? 1 : 0);
		player.getPackets().setIFText(WICKED_HOOD_INTER, 143, "" + player.getDailySubI("wickedRunes", wickedRunesCapacity));
	...
	public static ButtonClickHandler handleWickedHoodInter = new ButtonClickHandler(new Object[] { WICKED_HOOD_INTER }, e -> {
		if (e.getComponentId() == 119) {
			...
			int wickedEssCapacity = 100 + ((boolean) e.getPlayer().get("hasWickedCape") ? 25 : 0) + ((boolean) e.getPlayer().get("hasWickedBottom") ? 25 : 0) + ((boolean) e.getPlayer().get("hasWickedTop") ? 25 : 0);
			if (amountToTake > e.getPlayer().getDailySubI("wickedEss", wickedEssCapacity))
				amountToTake = e.getPlayer().getDailySubI("wickedEss", wickedEssCapacity);
			...
			if (e.getPlayer().getTempAttribs().getO("whr") == null)
				...
				if (selection != null) {
					...
					if (e.getPlayer().hasWickedHoodTalisman(selection)) {
						int wickedRunesCapacity = (1 + (e.getPlayer().getUsedElementalTalisman() ? 1 : 0) + ((boolean) e.getPlayer().get("hasWickedCape") ? 1 : 0) + ((boolean) e.getPlayer().get("hasWickedBottom") ? 1 : 0) + ((boolean) e.getPlayer().get("hasWickedTop") ? 1 : 0));
						if (e.getPlayer().getDailySubI("wickedRunes", wickedRunesCapacity) > 0 && e.getPlayer().getInventory().hasFreeSlots()) {
			...
			if (e.getPlayer().getTempAttribs().getO("whr") == null)
				...
				if (selection != null) {
					...
					if (e.getPlayer().hasWickedHoodTalisman(selection)) {
						int wickedTelesCapacity = 2 + ((boolean) e.getPlayer().get("hasWickedCape") && (boolean) e.getPlayer().get("hasWickedBottom") && (boolean) e.getPlayer().get("hasWickedTop") ? 1 : 0);
						if (e.getPlayer().getDailyI("wickedTeles") < wickedTelesCapacity) {
