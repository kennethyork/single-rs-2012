    ...
    fun searchBookcase(obj: GameObject) {
        player.schedule {
            ...
            if (obj.id == BOOKCASES[0]) {
                ...
                if(player.getQuestStage(Quest.THE_GOLEM) == STAGE_RETRIEVE_NOTES){
                        if (obj.id == BOOKCASES[1]) {
                            player.inventory.addItem(4616)
                            player.setQuestStage(Quest.THE_GOLEM, STAGE_LOOT_MUSEUM)
                            player.startConversation {
                                item(4616,"You find Varmen's expedition notes.")
                            }
                        }
                }else {
                    player.sendMessage("You find nothing of interest.")
