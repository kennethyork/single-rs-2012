            ...
            RangedWeapon.DEATHTOUCHED_DART -> {
                ...
                if (target is NPC) {
                    target.tasks.schedule(p.taskDelay) {
                        target.capDamage = -1
                        target.fakeHit(Hit(player, 10, HitLook.RANGE_CRIT).setText("<col=FF0000><shad=000000>INSTANT KILL"))
                        target.removeHitpoints(Hit(player, target.hitpoints, HitLook.TRUE_DAMAGE))
                    }
...
fun chargeDragonfireShield(target: Entity) {
    if (target is Player) {
        ...
        if (shield == 11283 || shield == 11284) {
            if (shield == 11284) {
                p.equipment.replace(Equipment.SHIELD, Item(11283, 1).addMetaData("dfsCharges", 1))
...
fun getAntifireLevel(target: Entity?, prayerWorks: Boolean): Int {
    ...
    if (target.hasEffect(Effect.ANTIFIRE)) {
        protection++
        if (shieldId == 1540 || shieldId == 11283 || shieldId == 11284 || shieldId == 16079 || shieldId == 16933){
            target.sendMessage("Your shield and potion fully protect you from the heat of the dragon's breath.", true)
        }else{
