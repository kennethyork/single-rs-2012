...
import static com.rs.engine.variables.VarBitPlayer.arena_lazy_guard;
			...
			case GET_JAIL_KEYS -> {
				...
				if(player.getInventory().containsItem(77)) {
					...
					addSimple("The guard quickly drinks half of the bottle and sways slightly.", () -> {
						...
						player.getVars().setVarBit(arena_lazy_guard, 1);
						WorldTasks.scheduleLooping(new Task() {
							...
							public void run() {
								...
								if (tick == 9) {
									...
									player.getVars().setVarBit(arena_lazy_guard, 2);
								}
								if(tick == 10)
									stop();
								tick++;
    ...
    public static NPCClickHandler handleDialogue = new NPCClickHandler(new Object[]{NPC, 7525, 7526, 7527}, e -> {
        ...
        if(e.getOption().equalsIgnoreCase("Steal-keys")) {
			if(e.getPlayer().getInventory().hasFreeSlots()) {
				...
				e.getPlayer().getVars().setVarBit(arena_lazy_guard, 3);
				return;
			}
			e.getPlayer().sendMessage("You need free space to grab his keys...");
		}
		if(e.getPlayer().getVars().getVarBit(arena_lazy_guard) > 0)
			return;
