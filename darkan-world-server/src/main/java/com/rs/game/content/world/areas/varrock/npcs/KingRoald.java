...
import com.rs.game.content.quests.allfiredup.npcs.KingRoaldKt;
...
public class KingRoald {
    ...
    public static NPCClickHandler handleKingRoald = new NPCClickHandler(new Object[]{ ID }, e ->  {
        Dialogue showQuests = new Dialogue().addOptions(questOptions -> {
            ...
            if (e.getPlayer().isQuestComplete(Quest.PRIEST_IN_PERIL) && e.getPlayer().getSkills().getLevel(Skills.FIREMAKING) >= 43)
                questOptions.add("About All Fired Up...", () -> KingRoaldKt.KingRoaldAllFiredUpD(e.getPlayer()));
