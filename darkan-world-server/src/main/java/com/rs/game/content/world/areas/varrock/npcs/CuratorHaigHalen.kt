...
import com.rs.game.content.quests.the_golem.curatorGolemDialogue
            ...
            if (player.questManager.getStage(Quest.DIG_SITE) in STAGE_GET_LETTER_STAMPED..STAGE_RECEIVED_SEALED_LETTER) {
                ...
                options {
                    ...
                    if(player.questManager.getStage(Quest.THE_GOLEM) == STAGE_LOOT_MUSEUM){
                        op("I'm looking for a statuette recovered from the city of Uzer."){
                            curatorGolemDialogue(npc)
                        }
                    }
                    op("Farewell.")
                }
...
fun mapCuratorHaigHalen() {
    ...
    onNpcClick(646, options = arrayOf("Pickpocket")) { (player) ->
        if(player.getQuestStage(Quest.THE_GOLEM) == STAGE_LOOT_MUSEUM){

            player.sendMessage("You attempt to pick the Curator's pocket...")
            player.schedule{
                player.lock()
                player.anim(881)
                wait(3)
                player.unlock()
                player.inventory.addItem(4617,1)
                player.startConversation{
                    item(4617, "You steal a tiny key.")
                }
            }
        }else {
