...
fun mapPKBotCombat() {
    ...
    onNpcClick(4336, options = arrayOf("Recruit", "Dismiss")) { event ->
        val bot = event.npc as? PKBotNPC ?: return@onNpcClick
        when (event.option) {
            "Recruit" -> SimulatedPlayerCompanionManager.recruit(event.player, bot)
            "Dismiss" -> SimulatedPlayerCompanionManager.dismiss(event.player, bot)
        }
    }
}
...
open class PKBotNPC(tile: Tile, private val definition: SimulatedPlayerDefinition? = null): NPC(4336, tile) {
    ...
    init {
        ...
        run = true
        definition?.combatLevel?.coerceIn(3, 138)?.let(::scaleCombatTo)
        ...
        definition?.combatLevel?.coerceIn(3, 138)?.let { combatLevel = it }
        when (definition?.style ?: Attack.MELEE) {
            Attack.MELEE -> maxMelee()
            Attack.RANGE -> maxRange()
            Attack.ICE_BARRAGE -> maxMage()
        }
        setRandomWalk(definition?.wander ?: true)
        updateAppearance()
        hitpoints = maxHitpoints
        setPermName("<col=FFFFFF>${definition?.name ?: randomUsername()}")
    }

    private fun scaleCombatTo(targetCombatLevel: Int) {
        val scaled = (targetCombatLevel * 0.72).toInt().coerceIn(3, 99)
        skills.keys.forEach { skills[it] = scaled }
        hpLevel = scaled
    }
    ...
    override fun canAggroPlayer(target: Player): Boolean =
        definition?.mode != SimulatedPlayerMode.SOCIAL && super.canAggroPlayer(target)

    override fun getDefinitions(): NPCDefinitions = addBotOptions(super.getDefinitions())

    override fun getDefinitions(player: Player): NPCDefinitions = addBotOptions(super.getDefinitions(player))

    private fun addBotOptions(definitions: NPCDefinitions): NPCDefinitions {
        val options = definitions.options ?: arrayOf("", "", "", "", "").also { definitions.options = it }
        listOf("Trade", "Recruit", "Dismiss").forEach { option ->
            if (options.any { option.equals(it, ignoreCase = true) }) return@forEach
            val emptySlot = options.indexOfFirst { it.isNullOrBlank() }
            if (emptySlot >= 0) options[emptySlot] = option
        }
        return definitions
    }

    /**
     * Social bots use the normal NPC movement engine but cannot be attacked or
     * pulled into interactions intended for combat NPCs.
     */
    override fun isCantInteract(): Boolean =
        definition?.mode == SimulatedPlayerMode.SOCIAL || super.isCantInteract()
    ...
    override fun drop(killer: Player?) {
        if (definition != null && !definition.dropsEquipment) return
        killer?.let {
