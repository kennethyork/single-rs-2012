...
public final class ChunkManager {
    ...
    public static void loadAllMapData() {
        ...
        if (preloadCollision) {
            for (int regionId = 0; regionId < 0xFFFF; regionId++) {
                ...
                if (!region.hasData())
                    continue;
                int baseChunkX = region.getBaseX() >> 3;
                int baseChunkY = region.getBaseY() >> 3;
                for (int plane = 0; plane < 4; plane++)
                    for (int cx = 0; cx < 8; cx++)
                        for (int cy = 0; cy < 8; cy++)
                            WorldCollision.initChunk(MapUtils.encode(Structure.CHUNK, baseChunkX + cx, baseChunkY + cy, plane));
                for (int x = 0; x < 64; x++)
                    for (int y = 0; y < 64; y++)
                        for (int plane = 0; plane < 4; plane++)
    ...
    public static void updateChunks(Entity entity) {
        ...
        if (entity.getLastChunkId() != chunkId || entity.isForceUpdateEntityRegion()) {
            if (!(entity instanceof Player player) || player.hasStarted())
