...
import com.rs.game.content.skills.magic.lunar.LunarSpellbook;
...
public class InventoryOptionsHandler {
	...
	public static boolean handleItemOnItem(Player player, Item used, Item usedWith, int fromSlot, int toSlot) {
		...
		if (usedId == 1759 && LunarSpellbook.getStrungIndex(usedWithId) != -1)
			if (player.getInventory().containsItem(1759, 1) && player.getInventory().containsItem(usedWithId, 1)) {
				...
				player.getInventory().addItem(LunarSpellbook.getStrung()[LunarSpellbook.getStrungIndex(usedWithId)], 1);
				return true;
			}
		...
		if (contains(1755, Gem.PEARL.getCut(), used, usedWith))
			GemTipCutting.cut(player, GemTips.PEARL);
	...
	public static void handleItemOption1(Player player, final int slotId, final int itemId, Item item) {
		...
		if (item.getId() == 407) { //Oysters
			player.getInventory().deleteItem(407, 1);
			final double random = Utils.random(100.0);
			if (random < 70.0){
				player.getInventory().addItem(411);
			}else{
				player.getInventory().addItem(409);
			}
		}
		if (item.getId() == 20120) {
