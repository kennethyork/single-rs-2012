	...
	public static void handleOptionExamine(final Player player, final GameObject object) {
		...
		if (player.hasRights(Rights.DEVELOPER)) {
			...
			if (object.getDefinitions().varpBit != -1)
				player.sendMessage("Transforms with varbit " + object.getDefinitions().varpBit + " - current obj: " + object.getDefinitions(player).id + " - current varbit: " + player.getVars().getVarBit(object.getDefinitions().varpBit));
