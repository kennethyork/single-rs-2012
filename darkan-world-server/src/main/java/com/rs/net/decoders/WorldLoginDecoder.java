...
public final class WorldLoginDecoder extends Decoder {
	...
	public int decodeWorldLogin(InputStream stream) {
		...
		if (Settings.getConfig().isSinglePlayer()) {
			a = LocalAccountStore.authenticate(username, password);
		} else try {
		...
		WorldDB.getPlayers().getByUsernameForLogin(account.getUsername(), pRes -> {
			try {
				if (pRes.loadError()) {
					// Existing player save could not be read/decoded from Mongo; do NOT create a
					// fresh player over the top of it - report an error and let them retry.
					session.sendClientPacket(13); // COULD_NOT_COMPLETE_LOGIN
					return;
				}
				final Player player = pRes.player() == null ? new Player(account) : pRes.player();
				...
				Logger.handle(WorldLoginDecoder.class, "decodeWorldLogin", "Error completing login for " + account.getUsername(), e);
				e.printStackTrace();
				session.sendClientPacket(13); // COULD_NOT_COMPLETE_LOGIN
			}
		});
