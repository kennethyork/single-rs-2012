				...
				if (getLoginStage() == LoginStage.UNK7_80) {
					...
					if (getLobbyStage() == LobbyStage.LOBBY_TO_WORLD) {
						...
						if (!Class9.socialNetworkLogin) {
							...
							i_6 = rsBB.index;
							// Offline mode skips the lobby, so send the entered password instead
							// of the lobby authentication token that would otherwise be null.
							rsbytebuffer_7 = ChatLine.getLoginMod(Boolean.getBoolean("darkan.singlePlayer"));
