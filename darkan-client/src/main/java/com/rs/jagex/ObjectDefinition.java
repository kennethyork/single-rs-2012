...
public class ObjectDefinition {
	...
	static PulseEvent nextPulseEvent() {
		...
		if (class282_sub50_sub12_1 != null) {
			...
			do {
				...
				if ((class282_sub50_sub12_1 == null) || (class282_sub50_sub12_1.getTime() > Utils.time()))
					return null;
				class282_sub50_sub12_1.unlink();
				class282_sub50_sub12_1.method13452();
	...
	public boolean hasSound() {
		...
		for (int element : transformTo)
			if (element != -1) {
				ObjectDefinition objectdefinitions_3 = loader.getObjectDefs(element);
	...
	public boolean method7968() {
		...
		synchronized (loader.meshIndex) {
			for (int[] modelId : modelIds)
				for (int i_5 = 0; i_5 < modelId.length; i_5++)
					bool_2 &= loader.meshIndex.load(modelId[i_5], 0);
			return bool_2;
		}
	}
