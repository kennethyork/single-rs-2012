...
public class client extends Engine {
	...
	public static IterableNodeMap GROUND_ITEMS;
	...
	static MenuAction[] aShortArray7239;
	...
	static {
		...
		anInt7262 = 0;
		anInt7376 = 0;
		Camera.camAngleX = 1052.0F;
		Camera.camAngleY = 0.0F;
		aFloat7266 = 0.0F;
		aFloat7284 = 0.0F;
		aBool7331 = false;
		aBool7269 = false;
		...
		aShortArray7239 = new MenuAction[] { MenuAction.OP_PLAYER1, MenuAction.OP_PLAYER2, MenuAction.OP_PLAYER3, MenuAction.OP_PLAYER4, MenuAction.OP_PLAYER5, MenuAction.OP_PLAYER6, MenuAction.OP_PLAYER7, MenuAction.OP_PLAYER8 };
		PLAYER_OPTION_CURSORS = new int[8];
		PLAYER_OPTIONS = new String[8];
		PLAYER_OPTION_REDUCED_PRIORITY = new boolean[8];
		anInt7311 = -1;
		GROUND_ITEMS = new IterableNodeMap(64);
		...
		anInt7429 = 0;
		Camera.SHAKING = new boolean[5];
		Camera.SHAKE_1 = new int[5];
		Camera.SHAKE_2 = new int[5];
		Camera.SHAKE_3 = new int[5];
		Camera.SHAKE_4 = new int[5];
		aShort7436 = 256;
		aShort7437 = 205;
		aShort7394 = 256;
		aShort7324 = 320;
			...
			if (!player_5.hasAppearance() || player_5.hidden)
				...
				if (player_5.aShort9458 >= 0 && player_5.aShort9456 >= 0 && player_5.localX < IndexLoaders.MAP_REGION_DECODER.getSizeX() && player_5.localY < IndexLoaders.MAP_REGION_DECODER.getSizeY()) {
					...
					if (player_5 == VertexNormal.MY_PLAYER)
						...
						if (!player_5.aBool10571 && !player_5.reducedAttackPriority) {
							if (anInt7291 == 0)
								i_8 += 32;
							else
								i_8 += 128;
	...
	static boolean method11651(IComponentDefinitions icomponentdefinitions_0) {
		if (aBool7168) {
			if ((getIComponentSettings(icomponentdefinitions_0).eventsHash != 0) || (icomponentdefinitions_0.type == ComponentType.CONTAINER))
				return false;
		}
			...
			if (iCompDef != null && i_2 == iCompDef.parent) {
				...
				if (iCompDef.type != ComponentType.CONTAINER && !iCompDef.usesScripts && getIComponentSettings(iCompDef).eventsHash == 0 && iCompDef != aClass118_7247 && !ContentType.isGameSceneRelated(iCompDef.contentType)) {
					...
					if (iCompDef.aBool1424 || leftBound < rightBound && lowerBound < upperBound) {
						...
						boolean clickable = iCompDef.clickMask && iCompDef.type == ComponentType.SPRITE && iCompDef.transparency == 0 && iCompDef.skyboxId < 0 && iCompDef.containerItemId == -1 && iCompDef.vorbisStringId == -1 && !iCompDef.tiling && iCompDef.angle2d == 0;
					...
					if (IS_USE_SELECTED && (Class506.USE_OPTIONS_FLAGS & 0x40) != 0) {
						...
						if (icomponentdefinitions_35 != null)
							PlayerModel.method4032(aString7275, " ->", Defaults8Loader.anInt5932, MenuAction.TGT_GROUND, iCompDef.containerItemId, 1L, i_33, i_34, true, false, iCompDef.slotId << 0 | iCompDef.idHash, true);
						else
							Class60.method1170();
						continue;
					}
					...
					if (Game.stellarDawn == CURRENT_GAME)
						PlayerModel.method4032(LocalizedText.FACE_HERE.translate(Class223.CURRENT_LANGUAGE), "", -1, MenuAction.FACE_SQUARE, -1, 1L, i_33, i_34, true, false, 0L, true);

					PlayerModel.method4032(Class85.aString817, "", anInt7311, MenuAction.WALK, -1, 1L, i_33, i_34, true, false, 0L, true);
					continue;
								}
								...
								if (iCompDef.contentType == ContentType.CONTENT_TYPE_1400) {
									...
									if (anInt7184 > 0 && !aBool7374)
										if (((anInt7339 == 1 || JS5GrabWorker.method12562()) && Class20.anInt169 > 2) || Class216.method3677())
						...
						if (iCompDef.type == ComponentType.SPRITE && iCompDef.skyboxId != -1)
	...
	void method11620() {
		if (GAME_STATE != GameState.UNK_16) {
			...
			else if (GameState.method4198(GAME_STATE) || GameState.method5789(GAME_STATE))
			...
			try {
				...
				if (updateStage == 3) {
					...
					if (Class47_Sub1.updateConnection.available(reponse)) {
						...
						for (Class446 element : arr_10)
							element.method7439(rsbytebuffer_4.readInt());
											...
											if (GAME_STATE == GameState.UNK_0 && !JS5CacheFile.method3360() || GAME_STATE == GameState.UNK_7 && Class9.anInt106 == 42 || GAME_STATE == GameState.IN_ACCOUNT_CREATION)
												if (GAME_STATE != GameState.UNK_7 && LOBBY_CONNECTION_CONTEXT.getConnection() == null)
													...
													if (LOBBY_CONNECTION_CONTEXT.anInt2290 > 50) {
														TCPPacket tcpmessage_18 = TCPPacket.createPacket(ClientProt.PING, LOBBY_CONNECTION_CONTEXT.outKeys);
